      ...
      <table class="table">
        ...
        <tbody>
          <tr v-if="filteredWasteProduct.length === 0">
            <td colspan="7" class="text-center">Không tìm thấy tồn kho</td>
          </tr>
          <tr v-for="(inventory, index) in paginatedWasteProduct" :key="inventory.sysIdPhePham">
            ...
            <td class="td-update-date">{{ new Date(inventory.ngayPhePham).toLocaleString('en-GB', {
              day: '2-digit',
              month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit'
            }).replace(/\/$/,
              '').replace(/:\d{2}$/, '') }}</td>
            <td class="td-quantity">{{ inventory.lyDo }}</td>
          </tr>
        </tbody>
      </table>
...
onMounted(() => {
  wasteProductStore.getWasteProduct();
});
...
const filteredWasteProduct = computed(() => {
  ...
  let filtered = wasteProductStore.wasteProduct.filter((product) => {
    return (
